...
// NOTA (v0.7.0 — simplificación): la búsqueda PROACTIVA hacia FUERA (conserje-fuera) se
// retiró. Adivinar la query pública desde el nombre de capacidad interno exigía un mapa
// cap→query calibrado a mano contra un catálogo que se mueve (frágil, ya se rompió). El
// valor de find-skills es que quien tiene la INTENCIÓN la escribe; por eso fuera se queda
// como PULL: las tools de chat buscar_fuera/traer_skill (feeder) pasan las palabras REALES
// del LLM —que ya conoce la tarea del turno vía el nervio— a skills.sh, sin traducción.
// El proactivo que queda (conserje-cantera) ofrece de la cantera PROPIA, indexada en
// español, sin adivinar ni red por tick. Menos pegamento, mejor resultado.
