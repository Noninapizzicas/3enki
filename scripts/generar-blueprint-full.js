#!/usr/bin/env node
/**
 * generar-blueprint-full.js — Blueprint SIN filtro de actor (TODAS las ops).
 * Superficie completa: consulta + supervisión + operativa (crear, editar,
 * enviar, configurar). No excluye ninguna operación del module.json.
 *
 * Uso: node scripts/generar-blueprint-full.js <slug> [--deploy] [--no-frontend]
 */
...
if (!target) {
  console.error('Uso: node scripts/generar-blueprint-full.js <slug> [--deploy] [--no-frontend]');
  process.exit(1);
}
