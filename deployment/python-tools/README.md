...
2. **SearXNG** (opcional) — el backend de búsqueda para `crawl4rs.buscar`.
...
Base `python:3.12-slim` + `requests` · `beautifulsoup4` · `lxml`.
...
## 2. SearXNG (opcional — solo para `crawl4rs.buscar`)

`leer`/`mapear`/`rastrear` NO lo necesitan. Solo la búsqueda de palabra-clave:
