...
| `arquitectura/cabecera/sistema-nervioso/bibliotecario.md` | aprendizaje | El puente a la biblioteca EXTERNA (repo Conocimiento): mirror git de solo-lectura de la bóveda Obsidian, servido por el bus como catálogo barato + préstamo bajo demanda. Reach-not-resident — el agente pide los libros que necesita, no carga el corpus. |
...
| `arquitectura/cabecera/plataforma/herramientas-externas.md` | plataforma | Órganos externos por naturaleza — Crawl4RS (web, Docker por Chromium) · OCR4RS (imagen/PDF escaneado, Rust puro NATIVO) · Python (SearXNG, Headroom, Docker). Las dos alas de la evidencia externa (web+físico). Enganche al ejecutor por config. |
