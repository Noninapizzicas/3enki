...
const REGLAS_PATH = 'entrega.json';

// Contrato entrega-v1 — null = política por declarar (el dueño puebla).
...
const FORMULAS = {
  tiempo: {
    descripcion: 'Estimación de tiempo total (preparación + reparto) para un pedido.',
    schema: {
    ...
    fn: (reglas, input) => {
      ...
      if (typeof base !== 'number' || typeof porItem !== 'number') {
        return { status: 200, data: { minutos_preparacion: null, minutos_reparto: null, minutos_total: null, metodo: 'pendiente', nota: 'estimación sin declarar' } };
      }
      ...
      if (rep.activo && typeof rep.minutos_por_km === 'number' && typeof input.km === 'number') {
        reparto = rep.minutos_por_km * input.km;
      }
      return { status: 200, data: { minutos_preparacion: round(preparacion), minutos_reparto: round(reparto), minutos_total: round(preparacion + reparto), metodo: 'declarado' } };
    }
  },
  reparto: {
    descripcion: 'Política de reparto: radio, coste, tiempo por km.',
    schema: {},
    fn: (reglas) => {
...
class EntregaReflejo extends ModuloHibridoReflejo {
  constructor() {
    super();
    this.name = 'entrega';
    ...
    this._custodio = ConfigCustodio.crear(this, {
      ...
      evento: 'entrega.reglas.actualizadas',
      campoDatos: 'reglas'
    });
  }

  // ================= RPC del bus (una línea por op — dispatch por id) =================
  onTiempoEstimarRequest(e) { return this._atender(e, 'tiempo_estimar', 'entrega.tiempo.estimar.response', d => this._calcular('tiempo', d)); }
  onRepartoObtenerRequest(e) { return this._atender(e, 'reparto_obtener', 'entrega.reparto.obtener.response', d => this._calcular('reparto', d)); }
  onReglasLeerRequest(e) { return this._atender(e, 'reglas_leer', 'entrega.reglas.leer.response', d => this._reglasLeer(d)); }
  onReglasActualizarRequest(e) { return this._atender(e, 'reglas_actualizar', 'entrega.reglas.actualizar.response', d => this._custodio.actualizar(d.project_id, d.cambios)); }
