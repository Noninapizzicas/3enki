...
async function run() {
  ...
  // 5) Fuente desconocida se ignora; sin fuentes → default (printables, thingiverse,
  //    makerworld, cults3d — printables y thingiverse primero por regla del dueño).
  mockFetch(HTMLS);
  ...
  assert.strictEqual(r.data.total, 5, 'sin fuentes → default (printables+thingiverse+makerworld+cults3d)');
  assert.strictEqual(r.data.por_fuente.thingiverse.n, 1, 'thingiverse se consulta por defecto (regla del dueño)');
  console.log('PASS 5: fuente desconocida ignorada; sin fuentes → default (thingiverse incluido)');
