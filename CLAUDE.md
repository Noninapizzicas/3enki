...
- **`_contratos/subsistema-tienda.contract.json`** — contrato transversal del subsistema-tienda (mayo 2026, v1.0.0). Canoniza la convención de PWA pública por proyecto servida por Caddy bajo `/tienda/<slug>`. Cada proyecto que active la feature `tienda` obtiene automáticamente: (a) directorio `public/tienda/` en su base_path, (b) symlink `/opt/enki/public/tiendas/<slug>` apuntando ahí, (c) Caddy lo sirve sin reload por convención de path. Origen empírico: conversación "New" del proyecto Vapers (2026-05-29) generó un bundle PWA con 15 sabores pero el usuario veía la versión vieja porque Caddyfile apuntaba al UUID del proyecto en lugar de al base_path real, y la URL `/vapers/` colisionaba con el namespace del frontend SvelteKit. El contrato cierra los 3 drifts simultáneamente: convención de path interno (`public/tienda/`), URL pública (`/tienda/<slug>`), separación frontend del sistema (`/<slug>/*`) vs PWA pública (`/tienda/<slug>/`). Módulos del subsistema (carta-digital, comandero-cliente-builder, cartadigital-pwa-builder) heredan del blueprint padre `subsistema-tienda.modulo-base` (path canónico del bundle, archivos canónicos: index.html/manifest.json/sw.js, evento canónico de cierre `tienda.bundle.actualizada`, actualización atómica). Módulos JS que escriben al bundle declaran `tienda_paths_escritos[]` en module.json (alternativa para los no-blueprint). Trabajo pendiente: `tienda.json` feature, blueprint padre, modificar Caddyfile (eliminar `/vapers/*` legacy + añadir `/tienda/*` canónico), validator, migración del bundle actual de Vapers, test runtime. Acoplado a `project-feature-blueprints` (tienda es una feature), `project-identity`, `events`, `modulos-blueprint-driven`, `security` (lista de archivos prohibidos en el bundle).
