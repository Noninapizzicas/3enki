...
when-to-use: Cuando llega una tarea/faceta nueva sin forma todavía ("gestionar X", "llevar Y", "coordinar Z") y hay que decidir QUÉ piezas construir antes de construirlas. También para auditar una parcela existente (¿cada pieza tiene la forma que le toca?). NO para ejecutar una parcela ya disecionada (eso lo hace su propia skill, p.ej. prisma-compuestos).
fuente: enki
dominio: metodo
