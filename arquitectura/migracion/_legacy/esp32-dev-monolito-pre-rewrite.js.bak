...
class ESP32DevModule {
  ...
  async handleCreateProject(data) {
    ...
    if (!project_name) return { status: 400, error: 'project_name requerido' };
    if (!template) return { status: 400, error: 'template requerido' };

    // Validar nombre
    if (!/^[a-z0-9][a-z0-9-]*$/.test(project_name)) {
      return { status: 400, error: 'project_name debe ser slug (lowercase, hyphens, empieza con alfanumérico)' };
    }

    // Verificar que no existe
    if (this.projects[project_name]) {
      return { status: 409, error: `Proyecto '${project_name}' ya existe` };
    }

    // Verificar template
    const tpl = this.templates.get(template);
    if (!tpl) {
      return { status: 404, error: `Template '${template}' no encontrado. Usa esp32.list-templates para ver disponibles.` };
    }
    ...
    if (!BOARDS[selectedBoard]) {
      return { status: 400, error: `Board '${selectedBoard}' no soportado. Usa esp32.list-boards para ver disponibles.` };
    }
    ...
    try {
      ...
      this.logger.error('esp32.project.create_failed', { project_name, error: err.message });
      return { status: 500, error: `Error creando proyecto: ${err.message}` };
    }
  }
  ...
  async handleGetProject(data) {
    ...
    if (!project) return { status: 404, error: `Proyecto '${project_name}' no encontrado` };
  ...
  async handleBuild(data) {
    ...
    if (this.activeBuilds.has(project_name)) {
      return { status: 409, error: `Proyecto '${project_name}' ya está compilando` };
    }
  ...
  async handleBuildStatus(data) {
    ...
    if (projectName) {
      ...
      if (!project) return { status: 404, error: `Proyecto '${projectName}' no encontrado` };
  ...
  async handleDeleteProject(data) {
    ...
    if (!project_name) return { status: 400, error: 'project_name requerido' };
    if (!confirm) return { status: 400, error: 'confirm: true requerido para eliminar' };

    const project = this.projects[project_name];
    if (!project) return { status: 404, error: `Proyecto '${project_name}' no encontrado` };

    if (this.activeBuilds.has(project_name)) {
      return { status: 409, error: `Proyecto '${project_name}' está compilando. Espera a que termine.` };
    }
