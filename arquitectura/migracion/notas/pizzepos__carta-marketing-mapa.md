# pizzepos__carta-marketing — Mapa exhaustivo (PASO 0 del rewrite)
...
- **Path**: `modules/pizzepos/carta-marketing/`
- **Version actual**: 2.0.0 → bump a **<TODO>** post-rewrite.
- **LOC index.js**: 472.
- **Drifts en baseline**: 34 (14 tipos).
- **Categoria**: core.
- **Description oficial**: Equipo de marketing silencioso pizzepos. Escucha carta.actualizada y dispara agentes especializados (marketing-copywriter, marketing-strategist, marketing-brand-keeper) cuando detecta necesidades. Loop prevention via hash. Perfil de marca persistido en {project.base_path}/storage/config/marca.json (write atomico).
...
- `agent.execute.request` — emitido en `onCartaActualizada (probable, vía detectarNecesidades dispatch)` (handler de `carta.actualizada`).
...
## Drifts conocidos en baseline (34)
...
1. Archivar monolito (472 LOC) en `_legacy/pizzepos__carta-marketing-monolito-pre-rewrite.js.bak`. _(automatico via scaffold)_
2. Reescribir `index.js` v<NEW> al canon:
   ...
   - Telemetria completa con prefix `carta-marketing.*`.
3. `module.json` v<NEW>:
...
4. Tests por capas (`tests/unit/pizzepos__carta-marketing.test.js` ya scaffoldeado):
