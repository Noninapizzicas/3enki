...
async function main() {
  ...
  // ============================================================
  // BLOQUE 2 — Propagación de los 9 campos en _executeToolCall
  // ============================================================
  console.log('\n[7] _executeToolCall enriquece args con los 9 campos');
  published.length = 0;

  // Lanzar tool call y capturar el evento publicado
  const callPromise = mod._executeToolCall('menu.generate',
  ...
  // Sin esperar a que se complete (no hay handler real). Capturamos el publish.
  await new Promise(r => setTimeout(r, 50));

  const toolPublishes = published.filter(p => p.event === 'menu.generate');
  assert(toolPublishes.length === 1, 'publica 1 evento menu.generate');
  const args = toolPublishes[0]?.data;
  assert(args.request_id, 'incluye request_id');
  ...
  // ============================================================
  // BLOQUE 3 — Args del LLM tienen prioridad sobre context
  // ============================================================
  console.log('\n[8] args del LLM tienen prioridad sobre el chat context');
  published.length = 0;

  mod._executeToolCall('menu.generate',
    { project_id: 'p-LLM', nombre: 'OtroNombre' },
    { project_id: 'p-CHAT', conversation_id: 'c-CHAT' }
  );
  await new Promise(r => setTimeout(r, 50));

  const args2 = published.filter(p => p.event === 'menu.generate')[0]?.data;
  ...
  // ============================================================
  // BLOQUE 4 — Sin chatContext, no rompe
  // ============================================================
  console.log('\n[9] _executeToolCall sin chatContext no rompe');
  published.length = 0;
  mod._executeToolCall('menu.generate', { nombre: 'X' }, undefined);
  await new Promise(r => setTimeout(r, 50));
  const args3 = published.filter(p => p.event === 'menu.generate')[0]?.data;
  assert(args3 != null, 'publicó algo aunque no haya chatContext');
  assert(args3.project_id === null, 'project_id es null por defecto');
  assert(args3._chat_context === null, '_chat_context es null por defecto');
