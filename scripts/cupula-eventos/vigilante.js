...
// ── cantos ───────────────────────────────────────────────────────────────────
const esRespuesta = (ev) => /\.(response|failed|respuesta)$/.test(ev) || ev.endsWith('.terminada') === false && false;
...
// ── reporte ──────────────────────────────────────────────────────────────────
if (AS_JSON) {
  console.log(JSON.stringify({ atendidos: atendidos.size, conducidos: conducidos.length, publicados: publicados.length, rpc_fantasma: errores, publish_huerfano: avisos }, null, 1));
} else {
