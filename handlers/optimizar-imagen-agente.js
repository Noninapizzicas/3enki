/**
 * Handler: Optimizar imagen con agente AI
 *
 * Cuando el OCR tiene baja confianza, dispara el agente image-processor
 * para que use las herramientas de Sharp y optimice la imagen.
 *
 * ENTRADA (evento): documento.ocr.completado (con baja confianza)
 *
 * SALIDA (evento): imagen.optimizar.request → agente → imagen.optimizada
 *
 * @version 1.0.0
 */
...
module.exports = [
  // =========================================================================
  // Detectar OCR con baja confianza → Disparar agente
  // =========================================================================
  {
    name: 'detectar-ocr-bajo',
    description: 'Detecta OCR con baja confianza y dispara agente optimizador',
    ...
    async handle(event, { logger, emit, services }) {
      ...
      try {
        ...
        logger.warn('optimizar-imagen-agente.info-error', { error: e.message });
      }

      // Disparar agente de optimización
      emit('imagen.optimizar.request', {
        filePath,
        ocrResult: {
          texto: texto?.substring(0, 500), // Limitar texto para contexto
          confianza
        },
        imageInfo,
        requestId,
