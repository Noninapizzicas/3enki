# Pasada 1 — Esquematizador-Jefe-Operador · catalogo-modelos

> SUJETO correcto: **las DOS caras trabajadas del catálogo de modelos 3D — la del JEFE
> (quien declara qué entra al catálogo y decide su calidad) y la del OPERADOR (quien
> consulta el catálogo a diario para saber qué hay disponible para imprimir)** — NO el
> módulo entero. La cara del CLIENTE (POS/PWA/consumo) se deja AL MARGEN: no es de este
> esquema.
>
...
> que elija modelos): el catálogo es un registro de alta + consulta. La dualidad aquí
> es débil — casi todo es jefe o neutro; la cara del OPERADOR es de CONSULTA (no hay
> operación física de máquina en este módulo, a diferencia de ciclo-impresion).
...
- **Confirmaciones**: `registrar` → `catalogo.modelo_registrado` (lleva modelo_id,
  nombre, categoria, project_id). Par de fallo: `catalogo.registrar.failed`.
...
1. **IDENTIDAD** — ¿Qué DECIDE el JEFE aquí? ¿Qué OPERA el OPERADOR aquí?
   ...
   - **OPERADOR (delgado)**: opera el taller a diario consultando el catálogo para saber
     qué modelos hay disponibles para imprimir (`listar`, `obtener`, `categorias`). No
     hay operación física de máquina aquí (eso es ciclo-impresion); la cara del operador
     es de CONSULTA.
   - **NEUTRO**: `listar`, `obtener`, `categorias` — solo informan.
2. **RESTRICCIONES** — ¿Qué NO depende de cada rol? El custodio del store es el PROPIO
   ...
   custodio es append-only hoy — no hay op de update/delete). El OPERADOR NO decide qué
   modelo entra (eso es del JEFE) — solo consulta lo ya registrado. El juez de la
...
3. **CONTRATO** — ¿Qué necesita VER cada rol antes de decidir/operar y qué SEÑAL pareada
   ...
   - VER (JEFE antes de registrar): `categorias` (distinct de listar, para el select de
     categoría) + `listar` (para no duplicar por id/nombre). VER (OPERADOR): `listar`
...
decide su calidad) → JEFE · ¿opera el flujo físico a diario (arranca, confirma físico,
retira, reanuda) → OPERADOR · ¿solo informa → NEUTRO? La cara CLIENTE (elegir/comprar)
NO se clasifica aquí.
...
| `listar` | neutro | Alimenta la cinta-estado y el ref-select de modelos (para `obtener`). Consulta del operador. |
| `obtener` | neutro | Detalle del modelo que alimenta la vista (metadatos, origen, archivo). Consulta del operador. |
| `categorias` | neutro | Distinct de listar — alimenta el select de categoría al registrar y el filtro del operador. |

**Dualidad en una línea**: el JEFE declara qué entra al catálogo (`registrar`) y decide
su calidad (aprobar/rechazar, [ABIERTO]); el OPERADOR consulta el catálogo a diario
(listar/obtener/categorias, neutro); el CLIENTE queda al margen (no hay cara de
utilización). El catálogo es más de GESTIÓN que de operación física.
...
3. DECLARAR/OPERAR — las ÚNICAS escrituras:
                  ...
                  · OPERADOR: no escribe — solo consulta (neutro).
```
...
- No hay acciones destructivas ni transiciones de estado en este módulo (el catálogo
  no tiene ciclo de vida de estados — solo alta + consulta). La cara del OPERADOR es
...
2. **Cinta del catálogo** — `cinta-estado` vía listar + categorias (consulta del
   operador).
...
- `ui.roles` = veredicto del árbitro: registrar=jefe, aprobar/rechazar=jefe [ABIERTO],
  listar=neutro, obtener=neutro, categorias=neutro.
- `ui.flujo` JEFE-PRIMERO: [jefe: registrar, aprobar/rechazar] → [neutro: listar,
  obtener, categorias]. El OPERADOR consulta (neutro), no escribe.
