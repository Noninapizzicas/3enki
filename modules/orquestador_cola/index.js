...
class OrquestadorColaReflejo extends ModuloHibridoReflejo {
  ...
  onAprobarCandidatoRequest(e)   { return this._atender(e, 'aprobar_candidato', 'orquestador_cola.aprobar_candidato.response', d => this._aprobarCandidato(d)); }
  ...
  async _alLiberarse(input) {
    ...
    // 2. GATE DE APROBACIÓN (regla del dueño): reimprimir algo ya impreso con éxito
    //    no necesita visto bueno humano → auto-aprueba y pasa a IMPRIMIENDO. Solo lo
    //    NUEVO (sin histórico por origen+ref) queda pendiente de aprobación.
    if (modelo.origen && modelo.ref) {
      const hist = await this._rpc('cola_modelos.ya_impreso.request', { project_id: input.project_id, origen: modelo.origen, ref: modelo.ref });
      const yaImpreso = hist && hist.status === 200 && hist.data && hist.data.ya_impreso;
      if (!yaImpreso) {
        // Modelo nuevo → no se imprime aún: se emite pendiente_aprobacion y se espera
        // la decisión del dueño (aprobar_candidato). La máquina queda en espera explícita.
        this._publicarEvento('cola.pendiente_aprobacion', { project_id: input.project_id, modelo, instante: nowISO() });
        return { status: 200, data: { ocupacion: 'pendiente_aprobacion', modelo } };
      }
    }

    // 3. Pasar a IMPRIMIENDO (el custodio valida el arco + el singleton 'imprimiendo').
    ...
    // 4. Emitir el evento de dominio y responder con la ocupación.
  ...
  // _aprobarCandidato: el dueño aprueba un modelo que quedó pendiente_aprobacion
  // (nuevo, sin histórico). Lo pasa a IMPRIMIENDO (el custodio valida arco + singleton).
  async _aprobarCandidato(input) {
    if (!input.project_id) return this._invalid('project_id');
    if (!input.id) return this._invalid('id');

    const trans = await this._rpc('cola_modelos.actualizar_estado.request', { project_id: input.project_id, id: input.id, estado: 'IMPRIMIENDO' });
    if (!trans) {
      return this._errorResponse(502, 'UPSTREAM_UNREACHABLE', 'no se pudo actualizar el estado del modelo', { id: input.id });
    }
    if (trans.status !== 200) {
      return this._errorResponse(trans.status, trans.error?.code || 'CONFLICT_STATE', trans.error?.message || 'no se pudo aprobar el candidato', { id: input.id });
    }
    this._publicarEvento('cola.candidato_aprobado', { project_id: input.project_id, id: input.id, instante: nowISO() });
    return { status: 200, data: { ocupacion: 'imprimiendo', modelo: trans.data?.modelo } };
  }
