...
class CobrosModule extends BaseModule {
  ...
  async handleCreateCobro(data) {
    try {
      ...
      return this._handleHandlerError('cobros.create.error', error);
    }
  }
  ...
  async publishCobroIniciado(cobro) {
    await this._publicarEvento('cobro.iniciado', {
      cobro_id: cobro.id,
      cuenta_id: cobro.cuenta_id,
      project_id: cobro.project_id || null,
      monto: cobro.monto,
  ...
  async publishCobroProcesado(cobro) {
    await this._publicarEvento('cobro.procesado', {
      cobro_id: cobro.id,
      cuenta_id: cobro.cuenta_id,
      project_id: cobro.project_id || null,
  ...
  async publishCobroReembolsado(cobro) {
    await this._publicarEvento('cobro.reembolsado', {
      cobro_id: cobro.id,
      cuenta_id: cobro.cuenta_id,
      project_id: cobro.project_id || null,
  ...
  async abrirCajonDinero(cobro) {
    try {
      ...
      await this._publicarEvento('periferico.abrir-cajon', {
        destino,
        pin: 0,
        project_id: cobro.project_id || null
      });
  ...
  _handleHandlerError(logEvent, err, kind = 'handler') {
    ...
    this.metrics?.increment?.('cobros.errors', { code, kind });
