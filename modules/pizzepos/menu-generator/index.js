...
class MenuGeneratorModule {
  constructor() {
    ...
    // Tarifas por canal: project_id → config
  ...
  async onProjectActivated(event) {
    ...
    // Track last active project for resolverPrecio calls without explicit projectId
...
Devuelve SOLO un JSON con este formato exacto, sin explicaciones:
{
  ...
  tarifasPathFor(projectId) {
    ...
    return path.join(paths.storagePath, 'config', 'tarifas.json');
  }
  ...
  async loadTarifas(projectId) {
    ...
    try {
      ...
      this.logger.info('menu-generator.tarifas.loaded', { project_id: projectId });
    } catch (err) {
      if (err.code !== 'ENOENT') {
        this.logger.warn('menu-generator.tarifas.load_error', { project_id: projectId, error: err.message });
      }
      // No file yet — use defaults (will be created on first save)
  ...
  async saveTarifas(projectId) {
    ...
    try {
      ...
      this.logger.info('menu-generator.tarifas.saved', { project_id: projectId });
    } catch (err) {
      this.logger.error('menu-generator.tarifas.save_error', { project_id: projectId, error: err.message });
    }
  }
   ...
   * @param {string} canal
   * @param {string} [projectId] - Si no se pasa, usa el último proyecto cargado
   * @returns {number}
   */
  resolverPrecio(producto, canal, projectId) {
    ...
    // Resolver config: proyecto explícito, o el último que haya cargado tarifas
  ...
  async toolSetTarifaCanal({ canal, multiplicador, recargo, project_id }) {
    ...
    this.metrics?.increment('menu.tarifa.canal.updated');

    this.logger.info('menu-generator.tarifa.canal.updated', {
      project_id, canal,
  ...
  async toolSetTarifaCategoria({ categoria, canales, multiplicador, recargo, project_id }) {
    ...
    // Si no se especifican canales, aplicar a todos los no-mesa
    ...
    this.tarifasPerProject.set(project_id, config);
    await this.saveTarifas(project_id);
    this.metrics?.increment('menu.tarifa.categoria.updated');

    this.logger.info('menu-generator.tarifa.categoria.updated', {
  ...
  async toolPreviewPrecios({ carta_id, producto_id, categoria, project_id }) {
    ...
    const CANALES = ['mesa', 'llevar', 'telefono', 'whatsapp', 'glovo', 'llevadoo'];
  ...
  async toolSetPrecioFijo({ carta_id, producto_ids, precio_fijo, project_id }) {
    ...
    await this.saveCartaToDisk(carta, project_id);
    this.metrics?.increment('menu.tarifa.precio_fijo.updated');

    // Propagar al POS para que comandero tenga el flag
    await this.eventBus.publish('carta.generada', { ...carta, project_id });
