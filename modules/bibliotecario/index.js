...
/**
 * bibliotecario — PUENTE A LA BÓVEDA DE CONOCIMIENTO (boveda/ en el repo).
 *
 ...
 *
 * La bóveda vive DENTRO del repo (boveda/). El bibliotecario lee directamente de ahí
 * — sin mirror git, sin clone. El agente/skill arranca LIGERO y pide prestados solo los
 ...
 *
 * Puertas (RPC del bus):
 *   bibliotecario.catalogo.request    {}                         → { sectores:[{sector,titulo,notas,dudosos}], total }
 *   bibliotecario.prestamo.request    { sector } | { consulta, topK? } → { libros:[{ruta,titulo,sector,cosechado,dudoso,cuerpo}], por }
 *   bibliotecario.sincronizar.request {}                         → { sincronizado, cambios, sectores, libros }
 */
...
class BibliotecarioModule extends ModuloHibridoReflejo {
  ...
  async onLoad(context) {
    ...
    this.logger?.info('bibliotecario.loaded', {
      ...
      sectores: this._catalogoCache.size, libros: this._libros
    });
  }
  ...
  _catalogo() {
    ...
    return {
      status: 200,
      data: { sectores, total: sectores.length, libros: this._libros }
    };
  }
  ...
  _prestar(d) {
    ...
    if (sector) {
      ...
      if (libros.length === 0) {
        return this._errorResponse(404, 'RESOURCE_NOT_FOUND', `sector sin libros: ${sector}`, {
          entity_type: 'sector', entity_id: sector
        });
      }
    } else {
      por = 'palabras';
    ...
    return { status: 200, data: { libros, por, total: libros.length } };
  }

  /** Reindexa la bóveda local y emite bibliotecario.actualizada si hay cambios. */
  async _sincronizar() {
    const start = Date.now();
    this.metrics?.increment('bibliotecario.sincronizar.total');
    const antes = this._libros;
    this._reindexar();
    const cambios = this._libros !== antes;

    if (cambios) {
      await this._publicarEvento('bibliotecario.actualizada', {
        sectores: this._catalogoCache.size,
        libros: this._libros,
        resumen: `biblioteca reindexada: ${this._catalogoCache.size} sectores, ${this._libros} libros`
      });
    }
    this.metrics?.timing('bibliotecario.sincronizar.duration_ms', Date.now() - start);
    return {
      status: 200,
      data: { sincronizado: true, cambios, sectores: this._catalogoCache.size, libros: this._libros }
    };
  }

  // =============================================================
  // Privados — escaneo del vault + parse de nota
  // =============================================================
