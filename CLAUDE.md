...
- **`_contratos/manual-mqtt-conexion-directa.contract.json`** — hermano del manual de audit-module. Cubre el nivel bajo: cuándo bajar a MQTT directo (cuando los helpers no bastan), datos de conexión (`wss://enki-ai.online/mqtt`, WebSocket sobre MQTT v3.1.1, sin auth en MQTT, bearer en HTTP para conversation-export), las dos familias de topics (la eventbus canónica `core/*/events/<dot.path>` para publish vs `core/+/events/<dot.path>` para subscribe — el `*` es literal y el `+` es wildcard MQTT; y la familia UI directa `ui/request/conversation/{create,send}` + `ui/response/<request_id>`), el envelope canónico (`event_id`, `event_type`, `timestamp`, `source.{core_id, module_id}`, `data.{request_id, correlation_id, ...campos_dominio}`, `metadata: {}`) con su shape derivado para topics UI directos (más plano), la mecánica de correlación request/response por `request_id`, las 4 secuencias canónicas (listar proyectos, crear conversación via topic UI, enviar mensaje y esperar respuesta vía eventbus, par request/response genérico de dominio), 5 probes de diagnóstico forense aplicados en audits reales (verificar persistencia tras timeout silencioso, aislar si filesystem es el culpable, inspeccionar tool_calls del LLM en `messages_raw[].metadata` para detectar payload vacío o alucinación, listening en vivo de un subsistema completo, verificar si un agente se invoca por `agent.execute.request`), catálogo de eventos read-only seguros vs mutaciones destructivas que clientes externos NO deben publicar, 7 errores clásicos al construir clientes ad-hoc (race subscribe/publish, clientId duplicado, olvido del setTimeout de seguridad, no filtrar `request_id`, JSON.parse sin try-catch, subscribir solo a `.response` ignorando `.failed` correlacionado, asumir que el timeline del export incluye payloads completos), e implementación de referencia mínima pegable en Node.js (snippet con `mqtt.connect` + subscribe con callback antes del publish + filtrado por `request_id` + setTimeout). Sin validator — manual operativo. Caso testigo: auditoría recetas 2026-05-20 que descubrió el bug del payload vacío en `bus.publishAndWait → fs.write.request` cuando el contenido del store es largo.
